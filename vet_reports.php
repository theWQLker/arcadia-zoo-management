...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    if (isset($_POST['add_report'])) {
        ...
        $weight_at_checkup = $_POST['weight_at_checkup'];
        $follow_up_date = $_POST['follow_up_date'];
        $habitat_comments = $_POST['habitat_comments'];

        $query = "INSERT INTO veterinary_reports (animal_id, vet_username, report_date, health_status, treatment, weight_at_checkup, follow_up_date, habitat_comments) 
                  VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
        $stmt = $pdo->prepare($query);
        $stmt->execute([$animal_id, $vet_username, $report_date, $health_status, $treatment, $weight_at_checkup, $follow_up_date, $habitat_comments]);
        ...
        $query = "UPDATE veterinary_reports 
                  SET animal_id = ?, vet_username = ?, report_date = ?, health_status = ?, treatment = ?, weight_at_checkup = ?, follow_up_date = ?, habitat_comments = ? 
                  WHERE report_id = ?";
        $stmt = $pdo->prepare($query);
        $stmt->execute([$animal_id, $vet_username, $report_date, $health_status, $treatment, $weight_at_checkup, $follow_up_date, $habitat_comments, $report_id]);
...
$query = "SELECT vr.report_id, vr.animal_id, a.prenom AS animal_name, vr.vet_username, u.name AS vet_name, 
          vr.report_date, vr.health_status, vr.treatment, vr.weight_at_checkup, vr.follow_up_date, vr.habitat_comments 
...
// Fetching the 4 most recent reports
$query_recent = "SELECT vr.report_id, vr.animal_id, a.prenom AS animal_name, vr.vet_username, u.name AS vet_name, 
          vr.report_date, vr.health_status, vr.treatment, vr.weight_at_checkup, vr.follow_up_date, vr.habitat_comments 
          FROM veterinary_reports vr
          JOIN animals a ON vr.animal_id = a.id
          JOIN users u ON vr.vet_username = u.username
          ORDER BY vr.report_date DESC LIMIT 4";
...
<body>
    <div class="container">
        ...
        <h2>Recent Veterinary Reports</h2>
        <table class="services-table">
            ...
            <?php foreach ($recent_reports as $report) : ?>
                <tr>
                    ...
                    <td><?php echo htmlspecialchars($report['weight_at_checkup']); ?></td>
                    <td><?php echo htmlspecialchars($report['follow_up_date']); ?></td>
                    <td><?php echo htmlspecialchars($report['habitat_comments']); ?></td>
                    <td>
        ...
        <form method="post" class="form-container">
            ...
            <label for="weight_at_checkup">Weight at Checkup:</label>
            <input type="number" id="weight_at_checkup" name="weight_at_checkup" value="<?php echo isset($_POST['weight_at_checkup']) ? htmlspecialchars($_POST['weight_at_checkup']) : ''; ?>"><br><br>
            ...
            <label for="habitat_comments">Habitat Comments:</label>
            <textarea id="habitat_comments" name="habitat_comments"><?php echo isset($_POST['habitat_comments']) ? htmlspecialchars($_POST['habitat_comments']) : ''; ?></textarea><br><br>
